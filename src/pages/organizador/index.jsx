...
import { eventoOrganizadorGet } from "../../models/eventos.model";

import Icon from "@/components/ui/Icon";
import Button from "@/components/ui/Button";
import cardImage1 from "@/assets/images/all-img/card-1.png";

const ContratarFotografo = () => {
    const [eventos, setEventos] = useState([]);
    const dataEvento = Array.from(eventos);
    useEffect(() => {
        const idUser = localStorage.getItem("user") || "{}";
        const userObj = JSON.parse(idUser);
        eventoOrganizadorGet(userObj._id).then(data => {
            console.log(data);
            setEventos(data);
        });
    }, []);
                    ...
                    <Card key={i} title={row.nombre} className="card-height-auto">
                        ...
                        <div className="mt-4 space-x-4 rtl:space-x-reverse">
                            <Button
                                ...
                                link={`/contratar-fotografo/${row._id}`}
                            />
                        </div>
                    </Card>
                ))}
...
export default ContratarFotografo;